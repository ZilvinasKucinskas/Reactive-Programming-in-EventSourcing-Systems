...
Sukūrus biblioteką, galime konkretizuoti taisykles, reikalingas srauto ir reaktyvių operatorių realizacijai. Šios taisyklės pateiktos diagramoje, kuri pavaizduota \ref{img:cappuccino_rules} paveikslėlyje. Iteratoriaus operatorius \lstinline|each(state_change_function)| yra praleistas, nes yra labai panašus į tipo atitikimo operatorių \lstinline|when(type, state_change_function)|. Vienintelis skirtumas, jog iteratoriaus operatorius netikrina tipo.

\textbf{Pagrindinės taisyklės}:

\begin{itemize}
  \item Kad galėtume naudoti reaktyvius operatorius, pirmiausia reikia sukurti srautą.
  \item Galima jungti skirtingus srautus.
  \item Srautui galima kviesti reaktyvius operatorius.
  \item Vienas srautas gali kurti daugiausiai vieną skaitymo modelį.
